...
*/
    $subject = 'Заявка с сайта Ёлки';
    $msg = '';
    $content .= '<hr>';

    if(isset($_POST['name'])) {
        $name = substr(htmlspecialchars(trim($_POST['height'])), 0, 100);
        $content .= '<b>Высота ёлки: </b>' . $name . '<br>';
    }
...
$mail = new PHPMailer();
$mail->AddAddress('egor.shishkin.official@yandex.ru');      	// кому - адрес, Имя (например, 'email@ rek9.ru','Денис Герасимов')
$mail->IsHTML(true);                        				// выставляем формат письма HTML
$mail->CharSet = "UTF-8";                   				// кодировка
$mail->From = "test@kazan.xn----8sbfkoelnbg5bbbyr.xn--p1ai";					        	// email, с которого отправиться письмо
$mail->FromName = "Ёлки";					    // откого письмо
