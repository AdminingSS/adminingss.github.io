...
*/
    $subject = 'Заявка с сайта Мамамебель';
    $msg = '';
    $content .= '<hr>';
    ...
    if(isset($_POST['complect'])) {
        $message = substr(htmlspecialchars(trim($_POST['complect'])), 0, 100);
        $content .= '<b>Комплект: </b>' . $message . '<br>';
    }
if(isset($_POST['parts'])) {
    $message = substr(htmlspecialchars(trim($_POST['parts'])), 0, 100);
    $content .= '<b>Части: </b>' . $message . '<br>';
}
if(isset($_POST['color'])) {
    $message = substr(htmlspecialchars(trim($_POST['color'])), 0, 100);
    $content .= '<b>Цвет: </b>' . $message . '<br>';
}
if(isset($_POST['sum'])) {
    $message = substr(htmlspecialchars(trim($_POST['sum'])), 0, 100);
    $content .= '<b>Стоимость: </b>' . $message . '<br>';
}
...
$mail = new PHPMailer();
$mail->AddAddress('lutsman.fl.ru@gmail.com');      	// кому - адрес, Имя (например, 'email@ rek9.ru','Денис Герасимов')
$mail->IsHTML(true);                        				// выставляем формат письма HTML
$mail->CharSet = "UTF-8";                   				// кодировка
$mail->From = "test@kazan.xn----8sbfkoelnbg5bbbyr.xn--p1ai";					        	// email, с которого отправиться письмо
$mail->FromName = "Мамамебель";					    // откого письмо
